...
<div style="display:none;" id="myDiv" class="animate-bottom fixed-position">
    ...
    <div class="row" style= "font-size:20px; padding:10px;">
        ...
        <div class="selector-bar">
            <div>
                <form method="post" accept-charset="utf-8" action="<?php echo site_url("insentif_cont/kcc_super"); ?>">
                    <select name="skcc_id" id="skcc_id" class="form" style="margin-right:25px;">
                        <option value=null>Select Super KCC</option>
                        <?php foreach ($super_kcc_list as $list) {?>
                            <option <?php if($super_kcc_id == $list["username"]){ echo "selected"; } ?> value="<?php echo $list["username"]; ?>"><?php echo $list["username"]." - ".$list["first_name"]." ".$list["last_name"]; ?></option>
                        <?php } ?>
                    </select>
