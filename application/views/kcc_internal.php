...
<div style="display:none;" id="myDiv" class="animate-bottom">
    ...
    <div class="row selector-bar" style="margin-bottom:30px;">
        <form method="post" accept-charset="utf-8" action="<?php echo site_url("insentif_cont/kcc_internal"); ?>">
            Supervisor :
            <select name="kcc_internal" id="kcc_internal" class="form" style="margin-right:25px;"> <!--onchange="this.form.submit()"-->
                <option value=null>Select Supervisor</option>
                <?php foreach ($kcc_internal_returned as $kcc_internal) { ?>
                    <option <?php if($kcc_internal_selected == $kcc_internal['username']){ echo "selected"; } ?> value="<?php echo $kcc_internal['username']; ?>"><?php echo $kcc_internal['username']." - ".$kcc_internal['first_name']." ".$kcc_internal['last_name']; ?></option>
                <?php } ?>
            </select>
        <!-- </form> -->
